...
class Scrobbler extends Controller {
	...
	{
		$jsonData = '{"s":"'.$sessionString.'","song":{"artistName":"'.html_entity_decode(urldecode($artistName)).'","track":"'.html_entity_decode(urldecode($track)).'","trackDuration":"'.$trackDuration.'","album":"'.html_entity_decode(urldecode($album)).'","mbTrackId":"'.$mbTrackId.'"}}';
		
		//$obj = json_decode($_POST["data"]);		
		$obj = json_decode($jsonData);
		
		//Check if session is valid and get username
		if( $this->User_Handshakes_Model->isHandeshakeValid($obj->{'s'}) )
		{
			$user_id = $this->User_Handshakes_Model->getUserIdForHandeshake($obj->{'s'});
			$artist_id = -1;
			
			//Verifica daca artistul este in baza si returneaza id-ul
			if( $this->Scrobbler_Model->isArtistInDB($obj->{'song'}->{'artistName'}) )
			{
				$artist_id = $this->Scrobbler_Model->getArtistIdByName($obj->{'song'}->{'artistName'});
			}
			else//in cazul in care nu este il adauga in baza
			{
				$artist_id = $this->Scrobbler_Model->insertArtist($obj->{'song'}->{'artistName'});
			}
			
			//Verifica daca daca track-ul este in baza
			if( $this->Scrobbler_Model->isTrackInDB($artist_id, $obj->{'song'}->{'track'}) )
			{
				$trackData = $this->Scrobbler_Model->getTrackData($artist_id, $obj->{'song'}->{'track'});
			...
			else
			{
				...
				{				
					$track_data = array(
					'artist_id' => $artist_id, 
				    'trackName' => $obj->{'song'}->{'track'},
				    'trackDuration' => $obj->{'song'}->{'trackDuration'},
					'album' => $obj->{'song'}->{'album'},
					'mbTrackId' => $obj->{'song'}->{'mbTrackId'}			    		
					);
